...
**프롬프트 전문**:
```
간밤 미국 기업 실적 '컨퍼런스콜' 내용을 정리해 페이지로 게시하고 카카오톡으로 발송하라(08:30 KST 2차 브리핑). 작업 공간에 xeob/briefing(공개)·xeob/briefing-secrets(비공개)가 클론돼 있다.

[무인 실행 — 최우선] 사람이 안 보는 예약 실행. 질문·선택지 제시하며 멈추지 말 것. 확인 불가는 쓰지 말고 생략(날조 절대 금지). publish_calls.sh를 통과 못 하면 발송 금지.

[★ 이 루틴의 존재 이유] 06:30 아침 브리핑 시점엔 컨퍼런스콜이 아직 진행 중이라(예: 테슬라 콜 06:30 시작~07:30 종료) 콜 내용을 담을 수 없다. 이 08:30 루틴이 '콜에서 나온 이야기'만 따로 전한다. **아침 브리핑에 이미 실은 수치(매출·EPS)를 반복하지 말 것** — 콜 발언·해설·특이사항 중심.

[중단 조건 — 중요] 다룰 컨콜이 없으면 **아무것도 발송하지 말고 그대로 종료**하라(빈 카드 금지). 구체적으로: earnings.py의 must_cover가 비었거나, 리서치 결과 콜 내용을 2소스로 확인한 종목이 하나도 없으면 → 게시·발송 없이 "발송 없음(사유)"만 보고하고 종료. 실적 시즌이 아닌 기간엔 이게 정상이다.
...
2) 대상 선정: deep(M7·반도체) 우선, 그 외 must_cover 중 시장이 크게 반응했거나 콜에서 주목할 발언이 나온 종목. 콜 내용을 **2소스로 확인 못 한 종목은 넣지 말 것**.
3) 리서치: 각 종목의 컨퍼런스콜 트랜스크립트·보도자료·주요 언론(2소스)에서 — ⓐ경영진 발언(수요·가격·공급·AI/capex 등) ⓑ가이던스 코멘트(다음 분기·연간) ⓒ특이사항(FCF·마진·일회성·규제·소송) ⓓ필요 시 시간외 주가 반응. **사실만, 출처 확인된 것만.**
4) pipeline/calls_template.html 구조·CSS 그대로 out/calls.html 생성. 헤더 날짜=오늘(한국시간, 예 2026.07.29 (수)). 한 종목=mv 블록 1개, 한글 종목명 (티커), 각 줄 40자 이내. '오늘 밤 실적 예정' 카드는 그날 장 마감 후 발표 예정인 화이트리스트 종목이 있을 때만, 없으면 카드 삭제.
5) out/calls_summary.txt 에 카톡 카드용 요약(150자 내외, 핵심 종목·키워드) 저장.
6) [게시·발송 = publish_calls.sh 로만] ./publish_calls.sh 실행. 내부 게이트(내용 없음·날짜 재탕)를 통과해야 발송된다. 아침 브리핑 index.html은 절대 건드리지 않는다(이 스크립트는 archive/<날짜>-calls.html만 추가).
