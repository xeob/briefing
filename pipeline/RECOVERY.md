...
3. **PAT 발급**: GitHub → Settings → Developer settings → Personal access tokens → **Fine-grained tokens** → Generate: Resource owner `xeob`, Repository access = **Only select repositories: `briefing`, `briefing-secrets`**, Permissions → **Contents: Read and write**. 만료 최장으로. 발급값 = `GH_PAT`.
   > ⚠️ **`briefing-secrets` 누락 주의 — 2026-08-03 전면 장애의 근본 원인.** 이 권한이 없으면 카카오가 회전시킨 새 refresh_token을 저장소에 되돌리지 못해 **다음 날부터 카톡 발송이 전부 실패**한다(KOE322). 게시는 계속 성공하므로 며칠간 모르고 지나가기 쉽다. 발급 후 `curl -H "Authorization: Bearer $GH_PAT" https://api.github.com/repos/xeob/briefing-secrets` 가 **200**인지 반드시 확인할 것(404면 권한 없음).
4. 파일 push는 §5에서.
...
#### 4.5b 2차 루틴 — 실적 컨퍼런스콜 브리핑 (08:30 KST)
06:30 브리핑 시점엔 컨콜이 진행 중이라(예: 테슬라 콜 06:30 시작~07:30 종료) 콜 내용을 담을 수 없다. 그래서 08:30에 콜 내용만 따로 발송한다.

| 항목 | 값 |
|---|---|
| 이름 / ID | Earnings Call Brief / trig_013HNiQDAFTf42VTCB9ALsx7 |
| 스케줄 | 매일 08:30 KST (cron `30 23 * * *` UTC) |
| 모델 | claude-opus-4-8 · 세션 유지 끔 |
| 소스·환경 | 아침 루틴과 동일(briefing + briefing-secrets) · env_01PB33HNQi33RBpK3eYeXi23 |
| 발송 | `publish_calls.sh` — 카카오 **기존 템플릿(134931) 재사용**, WDATE=`<날짜>-calls` 라 버튼이 컨콜 페이지로 감(새 템플릿 불필요) |
| 안전장치 | 다룰 컨콜 없으면 **발송 안 함**(빈 카드 금지) · 아침 index.html 미변경(archive만 추가) |

**프롬프트 전문**:
```
간밤 미국 기업 실적 및 '컨퍼런스콜' 내용을 정리해 페이지로 게시하고 카카오톡으로 발송하라(08:30 KST 2차 브리핑). 작업 공간에 xeob/briefing(공개)·xeob/briefing-secrets(비공개)가 클론돼 있다.

[무인 실행 — 최우선] 사람이 안 보는 예약 실행. 질문·선택지 제시하며 멈추지 말 것. 확인 불가는 쓰지 말고 생략(날조 절대 금지). publish_calls.sh를 통과 못 하면 발송 금지.

[★ 이 루틴의 존재 이유] 06:30 아침 브리핑 시점엔 컨퍼런스콜이 아직 진행 중이라(예: 테슬라 콜 06:30 시작~07:30 종료) 콜 내용을 담을 수 없다. 이 08:30 루틴이 그날 실적을 **더 자세히** 전한다.

[★★ 중복 환영 — 중요] 이 페이지는 **그 자체로 완결**돼야 한다. **아침 브리핑에 이미 실린 수치(매출·EPS 등)라도 반드시 다시 적는다.** 중복을 피하려고 수치를 생략하지 말 것(과거 사례: 수치 없이 발언만 실려 자료가 불완전했음). 아침=요약, 여기=상세 — **부문별 매출·세부 가이던스처럼 아침보다 더 깊게** 쓴다.

[중단 조건] 다룰 실적·컨콜이 없으면 **아무것도 발송하지 말고 그대로 종료**하라(빈 카드 금지). earnings.py의 must_cover가 비었거나, 2소스로 확인된 종목이 하나도 없으면 → 게시·발송 없이 "발송 없음(사유)"만 보고하고 종료. 실적 시즌이 아닌 기간엔 이게 정상이다.

0) 비밀값: set -a; source briefing-secrets/.env; set +a (값 출력 금지). 없으면 중단 보고.
1) briefing/pipeline 에서 python3 earnings.py 실행 → out/earnings.json. must_cover(그 세션 실적 발표 화이트리스트)와 deep(M7·반도체 = 심화 대상) 확인.
2) 대상 선정: **must_cover 전부를 기본**으로 하되, deep(M7·반도체)를 가장 자세히. 2소스로 확인 못 한 내용은 쓰지 말 것(단 수치는 보도자료·8-K로 확인 가능).
3) 리서치: 각 종목의 보도자료·8-K·컨콜 트랜스크립트·주요 언론(2소스)에서 — ⓐ**실적 수치**(매출·조정 EPS·예상 대비·부문별 매출·마진) ⓑ경영진 발언(수요·가격·공급·AI/capex) ⓒ가이던스(다음 분기·연간 — 수치까지) ⓓ특이사항(FCF·일회성·규제·소송) ⓔ시간외 주가 반응. **사실만, 출처 확인된 것만.** "검토·논의"를 "발표·결정"으로 격상 금지.
4) pipeline/calls_template.html 구조·CSS 그대로 out/calls.html 생성. 헤더 날짜=오늘(한국시간). 한 종목=mv 블록 1개, 한글 종목명 (티커), 각 줄 40자 이내. **mv-r 첫 줄은 반드시 실적 수치**(매출·조정 EPS·예상 병기). '오늘 밤 실적 예정' 카드는 해당 종목이 있을 때만, 없으면 카드 삭제.
5) out/calls_summary.txt 에 카톡 카드용 요약(150자 내외, 핵심 종목·수치) 저장.
6) [게시·발송 = publish_calls.sh 로만] ./publish_calls.sh 실행. 내부 게이트(내용 없음·날짜 재탕)를 통과해야 발송된다. 아침 브리핑 index.html은 절대 건드리지 않는다(archive/<날짜>-calls.html만 추가).
7) 보고: 게시 URL·카카오 result_code·다룬 종목 목록 + 제외 사유(2소스 미확인 등). 발송 없이 종료했다면 그 사유.
```
...
- **전일 미국 시장**:
  - **한 줄 요약(`sum`) + 원인 해설(`cmt`) — 그날 시장을 움직인 재료를 빠짐없이**:
    - **분량: 기본 2~3줄, 재료가 많은 날은 4~5줄까지 허용.** 정보손실 최소화가 분량보다 우선(일정·실적과 같은 원칙) — 줄 수 맞추려고 재료를 버리지 말 것.
    - **내부 정합성(엄수)**: **섹터 강세/약세의 사유로 지목한 재료는 해설에도 반드시 등장**시킨다. 예: 섹터에 "반도체 약세"를 쓰면 그 원인을 해설이 설명해야 한다. 같은 브리핑 안에서 해설·섹터·특징주가 따로 놀지 않게 한다.
    - **뉴스 섹션과 연결**: 「주요 뉴스」에 실은 미국 관련 헤드라인 중 **시장(지수·섹터·금리·환율)을 움직인 건은 해설에서도 다룬다.** 뉴스엔 있는데 해설엔 없어 서사가 끊기는 일 금지.
    - **AI·반도체 재료는 상시 점검 대상**: 한국 투자자에게 반도체·AI는 시장의 핵심 축이다. **비상장사 발(오픈AI·앤트로픽 등) 소식이라도 시장 영향이 크면 해설에 포함**한다 — 상장 예고·대규모 매출/전망 공개·빅테크 AI 투자 약정·공급망 계약 등. "실적 발표가 아니다"는 이유로 빼지 말 것. (실측 2026-08-20: 해설이 반도체 급락 원인으로 앤트로픽 전망을 들면서, 같은 날 뉴스에 실린 오픈AI 상장 예고·매출 공개는 누락.)
    - **지표가 '평소와 반대'로 받아들여진 날은 풀어서 설명(오해 방지)**: 「발표된 지표」 표의 색은 **지표 본래 의미** 기준이다(물가 예상 하회·소비/고용 예상 상회 = 초록). 그런데 그날 시장이 **반대로 반응**했으면(예: 긴축 국면이라 강한 소매판매가 금리 상승·주가 하락 재료) 결론만 적지 말고 **① 지표 본래 의미 → ② 그날 시장 국면 → ③ 전달 경로 → ④ 시장 결과**를 한 흐름으로 풀어 쓴다 — 표는 초록인데 해설은 악재라 독자가 헷갈리지 않게. 반대 방향(예: 고용 부진이 금리 인하 기대로 호재)도 같다.
      - ✗ "8월 소매판매가 전월비 1.2%(예상 0.8%)로 크게 웃돈 것도 긴축 명분을 키웠다."
      - ✓ "8월 소매판매는 전월비 1.2%(예상 0.8%)로 소비가 예상보다 강했다 — 평소라면 경기 호재지만, 연준이 인플레이션을 이유로 금리를 올리는 국면이라 '소비 과열 → 물가 압력 지속 → 추가 인상 근거'로 읽혀 금리를 끌어올렸다."
      (실측 2026-09-17: 표는 초록인데 해설은 "긴축 명분" 한 줄뿐이라 호재인지 악재인지 혼동.)
  ...
  - **특징주(핵심!) — 확정 규칙 (2026-07-02 사용자와 합의)**: 먼저 `python3 movers.py` 실행 → `out/movers.json`의 `qualified_up`/`qualified_down`(기본 25, **M7·메가캡 전원 + 일반 티어 |7%|+ 전원 강제 포함 — 이들은 25를 넘어도 잘리지 않음**).
    ...
    - **⑦ 사유 서술 품질 (개별 촉매·묶음 공통 이유 모두 적용)**:
      - **순환 설명 금지 — 결과를 원인으로 쓰지 말 것.** "비트코인 상승에 가상자산주 강세"·"금리 하락에 기술주 강세"처럼 **같은 층위의 결과**를 이유로 적으면 설명이 되지 않는다. **그 움직임을 만든 사건(정책·회의·발표·공시·지표·판결)까지 한 단계 더 파고들어** 적는다.
        - ✗ 나쁨: "비트코인 $68,000 회복 · SEC 규제안 발표"
        - ✓ 좋음: "백악관 크립토 서밋(코인베이스·리플·로빈후드 CEO 참석) — 트럼프 국가 차원 비트코인 매입 '검토' 발언 + SEC 가이드라인 공개"
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <!-- 원인 해설: 기본 2~3줄, 재료 많은 날 4~5줄까지. ★섹터 강세/약세 사유로 든 재료는 여기서도 반드시 설명하고,
         주요 뉴스에 실은 미국 건 중 시장을 움직인 것은 여기서도 다룰 것. AI·반도체는 비상장사(오픈AI·앤트로픽) 발이어도 포함.
         ★표의 지표 색(본래 의미)과 그날 시장 반응이 반대면(예: 긴축 국면의 강한 소매판매=악재) 결론만 쓰지 말고
         '본래 의미 → 시장 국면 → 전달 경로 → 결과'로 풀어 쓸 것. -->
    <p class="cmt">2~5줄 원인 해설.</p>
    ...
    <!-- 종목명은 반드시 한글 (티커 병기). 급등은 상승률 큰 순, 급락은 하락폭 큰 순.
         ...
         ★ 사유는 '결과'가 아니라 '사건'을 적는다 — "비트코인 상승에"·"금리 하락에" 같은 순환 설명 금지,
           그걸 만든 정책·회의·발표·공시·지표까지 파고들어 누가·무엇을·언제가 드러나게. 묶음 공통 이유도 동일.
           단 "검토·논의"를 "발표·결정"으로 격상 금지(2소스 확인된 사실만).
    ...
    <!-- ★ 브리핑이 다루는 미국장 세션 '당일'에 실제 발표된 지표만 넣는다(예: 7/2 세션 기준이면 7/2 발표분만). 다른 날 발표 지표 금지. 당일 발표가 없으면 이 표 대신 "· 당일 발표된 주요 지표 없음" 한 줄.
         ...
         — 지표 본래 의미 기준(그날 시장이 반대로 받아들였어도 색 유지, 그 사정은 미국 시장 해설에서 설명)
         ★ 기준 병기: events.json released 각 항목의 basis를 지표명에 함께 — "소비자물가지수 (6월, 전년비)".
           전년비·전월비가 둘 다 오면 둘 다 개별 행(전년비 먼저, 헤드라인 다음 근원).
           basis가 빈 항목(실업률·ISM·신규 실업수당 등)은 기준 없이 지표명만 — 고유 형태(수준·건수·지수) 그대로. -->
    <table class="tbl">
      <tr><th>지표</th><th>결과</th><th>예상</th></tr>
      <tr><td>소비자물가지수 (6월, 전년비)</td><td class="num"><span class="up">3.5%</span></td><td class="num">3.8%</td></tr>
    ...
    <!-- 통합 표 1개(지표·연준·실적·IPO 모두 행으로; 열·색태그·일시 형식은 아래 주석 참조).
         날짜순 정렬, 오늘 일정은 일시에 "오늘(0/0)" 표기.
         예외: FOMC·대형은행 실적 개막·스페이스X급 IPO는 1주 밖이어도 "예고" 허용.
         내용 없는 구분(예: IPO 없음)은 아래 회색 텍스트 줄로. 대형 IPO·상장 첫날 데뷔 필수 포함 -->
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <!-- ★ 한 종목 = mv 블록 1개. **이 페이지는 그 자체로 완결돼야 한다** —
           아침 브리핑에 이미 실린 수치라도 반드시 다시 적는다(중복 환영). 여기서 실적을 더 자세히 보는 게 목적.
         mv-h : 한글명 (티커) + 필요 시 시간외 주가 반응(up/down)
         mv-r : 첫 줄은 반드시 **실적 수치** — 매출 · 조정 EPS(예상 병기) · 필요 시 부문별 매출.
                이어서 컨콜 핵심 2~4줄(각 줄 40자 이내, 줄바꿈은 <br>):
                 - 경영진 발언(수요·가격·공급·AI capex 등)
                 - 가이던스(다음 분기·연간) — 수치까지 구체적으로
                 - 특이사항(FCF·마진·일회성·규제·소송 등)
         ★ 아침 브리핑보다 **더 자세히** 쓴다(아침=요약, 여기=상세). 부문별 실적·세부 가이던스 환영.
    ...
    <div class="mv">
      ...
      <div class="mv-r">매출 $28.2B(+26% YoY) · 조정 EPS $0.33 (예상 $0.53)<br>에너지 저장 $3.1B(+40%) · 자동차 총마진 17.2%<br>머스크: "AI·로보틱스 투자로 FCF 적자는 연말까지 지속"<br>연간 capex 가이던스 $25B+ 재확인 · 로보택시 확대 일정 언급</div>
    </div>
    <!-- 컨콜 확인된 종목 수만큼 반복 -->
  </div>
...
### 8.kakao_token.sh
> 카카오 토큰 갱신·회전토큰 방탄 저장(공통). 저장 후 `chmod +x`.
...
### 8.reauth_kakao.sh
> 카카오 재인증 1회용(사용자 실행). 저장 후 `chmod +x`.
