...
- **전일 미국 시장**:
  ...
  - **특징주(핵심!) — 확정 규칙 (2026-07-02 사용자와 합의)**: 먼저 `python3 movers.py` 실행 → `out/movers.json`의 `qualified_up`/`qualified_down`(각 최대 25, M7·메가캡 전원 + 일반 티어 |7%|+ 전원 강제 포함 — 메가캡이 많아도 큰 이동 일반주가 잘리지 않음).
    ...
    - **③ 이유 검증 (엄격)**: 후보를 우선순위 순서로 철저히 조사 — 수집·조사·검토·팩트체크를 다해 반드시 이유를 찾으려 노력. 그럼에도 못 찾으면: |등락률| 7% 미만 → **개별 탈락** / 7% 이상 → 유지하되 **"상승 이유 미확인"(급등) / "하락 이유 미확인"(급락)** 표기. 제외로 빈 슬롯은 다음 후보로 채움(backfill). 지어내기 절대 금지
      - **개별 탈락 종목의 행선지(명시)**: 그 종목이 **업종 동반 이동의 일부면 묶음행으로 수용**(업종 동반 자체가 이유) / **동반도 아니면 완전 제외**(이유 없는 단독 4~6% 이동은 노이즈로 간주 — 의도된 설계).
      - **촉매 "명확" 판정 기준**: 그 종목 **자체의 구체적 사건**으로, 기사·공시 출처로 확인 가능한 것 — 실적·가이던스 발표, 투자의견/목표가 변경, M&A·인수설(보도 확인), 대형 계약·수주, 규제·소송·FDA 결과, 신제품/기술 발표, 지수 편입/제외, 증자/자사주 등. **"불명확"** = 종목 자체 사건이 아닌 막연한 서술 — "업황 기대감", "AI 수혜", "차익실현", "저가 매수" 등 근거 기사 없이 붙일 수 있는 말. 업종 동반 이동은 개별 촉매가 아니라 **묶음 사유**다.
    - **⑤ 개별 선별 먼저(★ 순서 엄수)**: **동반 묶음보다 개별 특징주 선정을 항상 먼저** 한다. 급등·급락 각각 **개별 10개 목표**를 우선순위(M7·메가캡 → 뉴스 촉매 명확 → |등락률| 순)로 채운다. 10개를 억지로 채울 필요는 없지만, **개별 기준(등락폭·촉매·중요도)에 부합하는 종목은 반드시 개별로 넣는다.**
      - **★ 예외(대어 보장 — 정보손실 최소화)**: **|7%| 이상 + 촉매 명확** 종목은 우선순위 경쟁에서 밀려도 **개별 강제 게재** — 티어 1(M7·메가캡)이 많아 10을 넘게 되면 **그날은 10을 초과해서라도 전부 개별로** 싣는다(강한 날 3%대 메가캡이 +18% 실적 서프라이즈 종목을 밀어내 소멸시키는 것 방지 — 일정표 12행 초과 허용과 같은 원리).
    - **④ 업종 동반 예외 (개별 선정 '이후' 잔여분만)**: 개별 뉴스 없어도 동일 업종 동반 강세/약세면 포함, "업종 전반 강세/약세"로 설명. **묶음("그 외 ○○ 동반 강세/하락")은 위 개별 10에 이미 든 종목을 제외한 '나머지 동반 종목'만 대상으로 한다.** ★ 묶음에 넣기 전에 각 종목의 **개별 자격을 먼저 검토**: **개별 자격 = 티어 등락률 충족(그 외 ±4%/메가캡 ±3%/M7 ±2%) AND (개별 촉매 명확 OR |7%| 이상)**. 자격이 있으면 묶음이 아니라 개별로 — **4~7% 구간이라도 개별 촉매(자체 뉴스·실적·투자의견 등)가 명확하면 개별 후보이며, ⑤ 우선순위 경쟁(M7·메가캡 → 촉매 명확 → 등락률순)에 반드시 포함해 비교**한다(촉매 명확한 5%대가 촉매 미확인 9%보다 우선할 수 있음). **묶음에 남는 게 정당한 경우는 "티어 미달" 또는 "|7%| 미만 + 개별 촉매 없음(업종 동반만이 이유)"뿐.** verify.py(기계)가 차단하는 것은 촉매와 무관하게 확정인 한 경우: **개별 자리가 남는데(<10) |7%| 이상 종목이 묶음에 있을 때**. 4~7%+촉매 케이스는 기계 판단 불가 → 아래 사람판단 QA에서 확인. 개별 10이 꽉 찼으면 우선순위 판단으로 큰 %가 묶음에 남아도 무방. 각 종목 %를 up/down 색과 함께 병기. **줄바꿈: 종목명과 % 사이는 반드시 `&nbsp;`(예: `테라다인&nbsp;<span class="down">−13.6%</span>`), 나열 중간 억지 `<br>` 금지, `<br>`는 종목 나열과 '업종 공통 이유' 사이 딱 하나.**
  ...
  - 미국장 주요 일정(한국시간) — 섹션 제목도 이 이름 사용: **통합 표 1개(구분/내용/일시/비고)**. 구분 셀 색 태그: 지표=`tag s-ind`(파랑)·연준=`tag s-fed`(보라)·실적=`tag s-earn`(주황)·IPO=`tag s-ipo`(초록)·기타=`tag s-etc`(회색). 날짜순 정렬, 오늘 일정은 일시에 "오늘(0/0)". 일시는 날짜 위·시간 아래 줄분리(`7/6 (월)<br>23:00`). **표 목표 8~12행(비어닝시즌 기준). 단 1순위(지표·IPO)와 대상 실적(아래)이 많은 날은 12행을 넘겨서라도 전부 개별 게재 — 정보손실 최소화가 우선.** 잘리는 건 화이트리스트 밖 소형 실적·저우선 2순위뿐.
    ...
    - **실적 대상 = 아래 셋을 개별 포함(상한 없음)**: ① **M7**(애플·MS·아마존·구글·메타·엔비디아·테슬라) ② **메가캡($200B+)** ③ **경기 가늠자 화이트리스트(시총 작아도 경기 신호라 포함)** — 소비=리바이스·나이키·룰루레몬·스타벅스·맥도날드, 물류·항공=페덱스·UPS·델타·유나이티드, 산업=캐터필러·디어, 반도체(한국 관심)=마이크론·ASML·TSMC, 미디어=넷플릭스 등. 해당 종목이 in-window 실적 발표 시 **개별 행**으로, 비고에 역할(예: "소비경기 가늠자") 표기. **실적 상한 없음 — 몰리는 날은 12행 넘겨도 전부 개별 게재(요약 '그 외 다수'로 뭉개지 말 것).** 화이트리스트 밖 소형·비주력 실적만 제외. (화이트리스트는 상황 따라 RUN.md에서 갱신.)
...
**(B) 사람 판단 검증 — 스크립트가 못 잡는 것:** 뉴스 5분야 채움/시간창/출처 실존/중복 없음 · 키워드 근거 · 특징주 촉매의 사실 여부(팩트체크) · 촉매를 못 찾은 종목의 미확인 표기 정확 · **동반 묶음 안에 "티어 충족 + 개별 촉매 있는" 종목이 남아있지 않은지(있으면 개별 승격 — 특히 4~7% 구간은 기계가 못 잡으니 여기서 확인; 7%+&촉매 명확은 개별 10 초과해서라도 강제 게재)** · 날조 0. 문제 있으면 재수집·수정.
