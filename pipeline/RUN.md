...
- **전일 미국 시장**:
  ...
  - 미국장 주요 일정(한국시간) — 섹션 제목도 이 이름 사용: **통합 표 1개(구분/내용/일시/비고)**. 구분 셀 색 태그: 지표=`tag s-ind`(파랑)·연준=`tag s-fed`(보라)·실적=`tag s-earn`(주황)·IPO=`tag s-ipo`(초록)·기타=`tag s-etc`(회색). **범위=오늘부터 1주일**, 날짜순 정렬, 오늘 일정은 일시에 "오늘(0/0)". **최대 8행**. 일시는 날짜 위·시간 아래 줄분리(`7/6 (월)<br>23:00`).
    - **★ 포함 우선순위 (명확한 기준 — 반드시 이 순서로 채운다)**:
      - **1순위 = 무조건 포함(in-window면 절대 누락 금지)** — 판단 기준은 **"이 일정이 시장(지수·환율·금리·유가·업종)을 움직일 수 있는가". 그렇다면 아래 예시에 없어도 반드시 넣는다.** ⓐ 핵심 지표 — **CPI·PCE·고용보고서(NFP)·FOMC(회의/의사록/의장·연준위원 발언)·GDP·소매판매·ISM·PPI·소비심리** ⓑ 정책·거시 이벤트 — **관세/무역협상 발표, 주요국 중앙은행 결정, OPEC+, 국채 입찰, 옵션·선물 동시만기(쿼드러플 위칭), 부채한도·셧다운** 등 ⓒ 대형·화제 IPO·상장, 특히 **한국 관련(예: SK하이닉스 ADR 상장)** ⓓ 어닝시즌 개막 대형은행·메가캡 실적, **대형 M&A·규제(FDA·반독점) 결정**. (FOMC·대형은행 개막·초대형 IPO는 1주 밖이어도 1~2행 "예고" 허용.)
