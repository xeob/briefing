...
2. **뉴스 날짜창 엄수.** 각 뉴스는 URL/게재시각을 확인해 **전일~당일 발행분만**. **전전일(2일 전) 이전 뉴스는 절대 포함 금지.** 국내 뉴스는 전일 장중 포함 가능, 해외·야간 소식은 전일 22:00 이후 우선. verify.py가 URL 날짜로 구뉴스를 차단하므로 통과할 때까지 교체.
...
> 시간 창: 루틴은 매일 **06:00 KST** 실행. 뉴스는 **전일~당일 발행분만**(전전일 이전 금지). 국내 뉴스는 전일 장중 포함 가능, 해외·야간 소식은 전일 22:00 이후 우선. 미국 시장은 **직전 개장 세션**(전일 16:00 ET 마감 = 당일 05:00 KST, 휴장이면 그 이전 개장일) 기준. 헤더 설명글은 "전일 22:00 ~ 당일 06:00"으로 유지.

- **주요 뉴스** — 5개 분야(정치·경제·세계·IT·산업), 분야당 3~5건.
  5단계 선별(수집→클러스터/중복제거→잡음 필터→점수화[영향범위·파급·신규성·보도밀도·시의성]→요약+검증). 각 헤드라인에 원문 URL. **날짜창 엄수: 전전일(2일 전) 이전 기사 금지 — verify.py가 URL 날짜로 차단하니 창 밖이면 최신 기사로 교체.**
