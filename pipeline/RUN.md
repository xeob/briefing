...
- **전일 미국 시장**:
  ...
  - **특징주(핵심!) — 확정 규칙 (2026-07-02 사용자와 합의)**: 먼저 `python3 movers.py` 실행 → `out/movers.json`의 `qualified_up`/`qualified_down`(각 최대 25, M7·메가캡 전원 + 일반 티어 |7%|+ 전원 강제 포함 — 메가캡이 많아도 큰 이동 일반주가 잘리지 않음).
    ...
    - **⑤ 개별 선별 먼저(★ 순서 엄수 — 깐깐하게, "정말 의미 있는 특징주"만)**: **동반 묶음보다 개별 선정을 항상 먼저** 한다. 급등·급락 각각 **6~10개 목표** — 의미 기준 미달이면 **억지로 채우지 말 것**(낯선 종목으로 10을 채우는 것 금지). 세부 정렬:
      1. **M7** — 항상 개별 (시장 전체 영향)
      2. **메가캡($200B+)** — 항상 개별
      3. **비메가캡 촉매 종목 — 촉매 등급으로 차등**: **A급(확정 사실 이벤트)** = 실적/가이던스 발표·M&A 확정/공식 제안·규제/FDA/판결 결과·대형 계약 공시·상장/지수 편입 → **B급(2차 정보)** = 투자의견/목표가 변경·언론 보도/루머·애널 코멘트. A급 먼저, 같은 등급이면 |등락률| 큰 순 → 시총 큰 순.
      4. **촉매 미확인 |7%|+** — 맨 뒤 (시총·거래대금 큰 순).
      - **★ 낯선 중소형 게이트**: 시총 **$50B 미만** 종목은 **A급 촉매 또는 |10%| 이상**이 아니면 개별 게재하지 않는다(정말 좋은 뉴스·의미 있는 이동만). M7·메가캡의 움직임이 항상 우선.
      - 대어 보장 유지: |7%|+ & A급 촉매는 우선순위에서 밀려도 개별 강제(10 초과 허용).
