...
- **전일 미국 시장**:
  ...
  - **특징주(핵심!) — 확정 규칙 (2026-07-02 사용자와 합의)**: 먼저 `python3 movers.py` 실행 → `out/movers.json`의 `qualified_up`/`qualified_down`(각 최대 25, M7·메가캡 전원 + 일반 티어 |7%|+ 전원 강제 포함 — 메가캡이 많아도 큰 이동 일반주가 잘리지 않음).
    ...
    - **④ 업종 동반 예외 (개별 선정 '이후' 잔여분만)**: 개별 뉴스 없어도 동일 업종 동반 강세/약세면 포함, "업종 전반 강세/약세"로 설명. **묶음("그 외 ○○ 동반 강세/하락")은 위 개별 10에 이미 든 종목을 제외한 '나머지 동반 종목'만 대상으로 한다.** ★ 묶음에 넣기 전에 그 종목이 개별 기준에 부합하는지 먼저 검토 — 부합하면 묶음이 아니라 개별로. **단, 개별 기준 미달 종목(|7%| 미만 + 개별 촉매 없음)은 등락률이 개별 최소보다 커도 묶음에 남는 게 정당**(업종 동반이 그 종목의 이유). verify.py가 차단하는 것은 딱 한 경우: **개별 자리가 남는데(<10) |7%| 이상 종목이 묶음에 있을 때**(7%+는 촉매 못 찾아도 "이유 미확인"으로 개별 유지 자격이므로 승격 필요). 개별 10이 꽉 찼으면 우선순위(M7·촉매·중요도) 판단으로 큰 %가 묶음에 남아도 무방. 각 종목 %를 up/down 색과 함께 병기. **줄바꿈: 종목명과 % 사이는 반드시 `&nbsp;`(예: `테라다인&nbsp;<span class="down">−13.6%</span>`), 나열 중간 억지 `<br>` 금지, `<br>`는 종목 나열과 '업종 공통 이유' 사이 딱 하나.**
